...
export function migrateSeminarPage(doc) {
...
if(process.argv.includes('--apply')){
 ...
 const q='*[_type=="sitePage" && _id in ["page-home","page-seminars","drafts.page-home","drafts.page-seminars"]]';
 ...
 const docs=await read();assert.ok(docs.some(d=>d._id==='page-home'));assert.ok(docs.some(d=>d._id==='page-seminars'));
 const backup=process.env.SEMINAR_MIGRATION_BACKUP;if(!backup)throw new Error('Set SEMINAR_MIGRATION_BACKUP to a backup file path.');fs.writeFileSync(backup,JSON.stringify(docs,null,2));
 const updates=docs.map(migrateSeminarPage);const mutations=updates.map((d,i)=>({patch:{id:d._id,ifRevisionID:docs[i]._rev,set:{sections:d.sections}}})).filter((m,i)=>JSON.stringify(updates[i].sections)!==JSON.stringify(docs[i].sections));
 ...
 const saved=await read();for(const d of updates)assert.deepEqual(saved.find(x=>x._id===d._id).sections,d.sections);console.log('Verified seminar entries for',saved.map(d=>d._id).join(', '));
}
