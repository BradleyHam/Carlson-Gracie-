...
export function renderCoaches(coaches, config) {
  ...
  return coaches.map(coach => {
    ...
    return '<article class="coach"><div class="coach-photo'+(!image?' ph':'')+'">'+(image ? '<img src="'+escape(image)+'" alt="'+escape(coach.alt || coach.name)+'" loading="lazy">' : escape(coach.placeholder || 'Portrait to add'))+'</div>'+belt+'<h3>'+escape(coach.name)+'</h3>'+(coach.role ? '<p class="coach-rank">'+escape(coach.role)+'</p>' : '')+(coach.bio ? '<p class="coach-bio">'+escape(coach.bio).replace(/\n/g,'<br>')+'</p>' : '')+'</article>';
  }).join('');
}
