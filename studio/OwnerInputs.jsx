...
export function UsageInput() {
 const client=useClient({apiVersion:'2025-02-19'});const id=useFormValue(['_id']);const [pages,setPages]=useState(null);const [error,setError]=useState(false);
...
export function OwnerGuide() {
 return <div style={{...stack,gap:28,padding:32,maxWidth:820,boxSizing:'border-box',overflowY:'auto',height:'100%'}}>
  ...
  {[['Write a blog article','Open Blog posts → Create. Add the title, generate its address, choose News or Technique, add a cover image and write the article. Publish when ready. It appears in its category and the latest three appear on Home. Unpublish to remove it. Website updates take a few minutes.'],['Update a coach','Open Coaches. Edit one profile, then Publish. Pages select these shared profiles; you can change the selection and order in each page’s coaches section.'],['Update a seminar guest','Open Seminar guests. Keep their name, credentials and photo together. Choose which guests appear on Home and Seminars.'],['Add a dated event','Open Events → Upcoming & current → Create. Enter the date, location, booking link and photo, then Publish. Completed events move into Past events.'],['Edit a page','Open Pages or Academies and choose the page. Sections follow the website from top to bottom. Text is editable directly; photo entries show thumbnails.'],['Publish safely','Edits stay in drafts until you Publish. Open the website to check the result; published updates can take a short time to appear.'],['Shared content and page-specific details','A profile is the main source for a person. Some existing pages had different wording or crops; these are preserved together on that profile under Page-specific details. Clear an optional override to use the main profile value.'],['Remove content','Removing a profile from a page affects only that page. Turn off Show on website on a profile to hide it everywhere. Strong references protect profiles that are still selected.'],['Class timetable','The timetable remains managed separately through the academy’s timetable workflow. Event dates do not change weekly classes.']].map(([title,body])=><section style={{...stack,gap:8,flexShrink:0}} key={title}><h2 style={{margin:0,fontSize:17,lineHeight:1.4}}>{title}</h2><p style={paragraph}>{body}</p></section>)}
 </div>;
}
